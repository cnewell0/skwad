...
final class AgentManager {
           ...
           let command = SlashCommandCatalog.locallyHandled(trimmed, agentType: agent.agentType) {
            ...
            if command.name == "model" {
                ...
                if argument.isEmpty {
                    ...
                    // The agent's own "Set model to X" reply arrives via the
                    // transcript and renders as a report; a second confirmation layer
                    // here produced duplicates and false "did not confirm" cards.
    ...
    func answerChoice(_ index: Int, question: String? = nil, for agentId: UUID) {
        ...
        if let openChoice, openChoice.text.hasPrefix(Self.modelChoiceQuestionPrefix) {
            ...
            // The card has been answered, so it should not stay on screen
